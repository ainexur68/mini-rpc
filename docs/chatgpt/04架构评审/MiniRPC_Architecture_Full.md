...
固定头字段如下（共 22 字节，对齐《MiniRPC 1.0 需求冻结文档》与 PROTOCOL.md）：

| 字段名         | 长度 | 类型   | 描述                                      |
|---------------|------|--------|-------------------------------------------|
| Magic         | 2B   | short  | 协议魔数，固定为 `0xCAFE`，用于快速过滤非法数据包 |
| Version       | 1B   | byte   | 协议版本号，MiniRPC 1.0 固定为 `1`          |
| SerializeType | 1B   | byte   | 序列化方式（0=JSON，1=Kryo，2–127 保留，128–255 用户扩展） |
| Flags         | 2B   | short  | 16 位 bit 位标志（心跳、压缩、加密、单向、响应等） |
| RequestId     | 8B   | long   | 请求 ID，用于在异步场景下匹配响应               |
| HeaderLength  | 4B   | int    | 扩展头长度，单位字节，可为 0                  |
| BodyLength    | 4B   | int    | Body 长度，单位字节，可为 0                   |
...
## 12. 附录：错误码与异常映射 / 调用生命周期 / 模块依赖约束

### 12.1 错误码与异常映射

为避免实现阶段各层“各自定义一套错误处理”，MiniRPC 在架构层统一约定：

| 层级                     | 典型异常/场景                                      | 协议错误码（RpcResponse.errorCode） |
|--------------------------|---------------------------------------------------|-------------------------------------|
| Client Future / Filter   | `TimeoutException`（Future 超时）                 | `TIMEOUT`                           |
| Serializer.encode        | JSON/Kryo 序列化失败                              | `SERIALIZE_ERROR`                   |
| Serializer.decode        | JSON/Kryo 反序列化失败                            | `DESERIALIZE_ERROR`                 |
| Protocol Decoder         | Magic/Version 不合法，BodyLength 越界             | `PROTOCOL_ERROR`                    |
| Transport / Channel 层   | 连接已关闭、写入失败                              | `CONNECTION_CLOSED`                 |
| Provider 业务执行        | 未捕获 RuntimeException                           | `INTERNAL_SERVER_ERROR`             |
| SerializeType 不支持     | 未注册的序列化实现                                | `UNSUPPORTED_SERIALIZE_TYPE`        |

**约束：**

- 所有异常最终都映射为上述错误码之一，禁止在协议层引入新的整型错误码。
- `RpcResponse.errorMessage` 用于携带人类可读的错误描述，方便日志排查。
- 客户端 SDK 对外只暴露少量业务友好的异常类型（如 `RpcTimeoutException`），内部再映射到错误码。

### 12.2 调用生命周期（Client 侧）

1. 业务线程发起接口调用，经由动态代理进入 `Invoker`。  
2. Filter 链依次执行：注入 TraceId、记录调用日志、应用超时/重试配置。  
3. 根据服务名从本地服务缓存中获取 Provider 实例列表，如缓存为空则向 Registry 拉取。  
4. `LoadBalancer` 基于实例列表和本次调用（可包含 attachments）选择具体实例。  
5. 从 ChannelPool 获取或创建连接，将 `RpcRequest` 编码为 MiniRPC 帧并写入 Netty Channel。  
6. 将一个 `CompletableFuture` 放入 inflight 请求表（key 为 RequestId）。  
7. 如果在超时时间内收到响应帧：  
   - Decoder 解码为 `RpcResponse`，根据 `errorCode` 决定是正常返回还是异常抛出；  
   - 从 inflight 表中移除对应 Future，并完成。  
8. 如果超时仍未收到响应：  
   - 从 inflight 表移除 Future，标记为 `TIMEOUT`；  
   - **即便之后迟到的响应到达，也只记录日志并丢弃，不再回填给业务。**

### 12.3 Provider 生命周期与注册中心交互

**启动流程：**

1. 读取本地配置，启动 Netty Server，监听指定端口。  
2. 将自身实例信息（`ip:port`）注册到 Redis：写入 `mini_rpc:service:{serviceName}` 集合，并设置合理 TTL。  
3. 周期性续期 TTL，避免因短暂 GC/STW 导致实例被误删。  

**优雅下线流程：**

1. 停止接受新请求（可选：在治理层加入“摘流”开关）。  
2. 从 Redis 对应服务集合中移除当前实例。  
3. 通过 `PUBLISH mini_rpc:notify {serviceName}:OFFLINE` 发送一次变更通知。  
4. 关闭 Netty Server 和业务资源。  
5. TTL 机制仅作为兜底，而不是唯一的摘除手段。  

Consumer 侧：

- 通过订阅 `mini_rpc:notify` 频道感知服务变更，更新本地缓存。  
- 若长时间未收到通知，可定期主动全量拉取一次服务列表作为补偿。  

### 12.4 模块依赖与边界约束（防止实现阶段越界）

为避免实现阶段“顺手跨层访问”，架构层明确以下边界约束：

1. **Filter / 治理层：**
   - 只操作 `RpcRequest` / `RpcResponse` / `RpcContext` 等“上层抽象对象”；
   - 不允许直接访问 Netty `ByteBuf`、协议固定头字段等低层细节；
   - 若需要携带 TraceId、路由标签等信息，应通过 `attachments` 写入，由 Protocol 层负责落入 Ext Header。

2. **LoadBalancer：**
   - 接口只依赖抽象 `ServiceInstance`（包含 host、port、metadata）；
   - 不允许直接访问 Redis 客户端、Netty Channel 等；
   - 通过 `Invocation` 或 `attachments` 获取必要的路由信息（例如灰度 tag）。

3. **Registry：**
   - 只负责服务实例的注册、发现、变更通知；
   - 对调用协议、序列化、传输实现保持无感知；
   - 对上暴露的仅是 `List<ServiceInstance>` 与事件流。

4. **Transport：**
   - 只负责连接生命周期管理、读写 MiniRPC 帧；
   - 不感知业务方法、路由规则、错误码语义。

上述约束在实际实现中对应为：通过独立模块（Maven module）+ 明确的 Java 接口划分来落实，禁止“跨模块引用实现类”。
